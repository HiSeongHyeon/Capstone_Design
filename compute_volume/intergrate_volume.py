"""
데이터로부터 부피를 측정하는 통합 스크립트

이 스크립트는 findMask와 linear_RBF의 computeVolume을 사용하여
...
from tools.linear_RBF import computeVolume
...
def measure_volume_from_data(
    ...
    cup_diameter_mm : float, optional
        ...
        주의: linear_RBF는 평균 깊이 기반 스케일을 사용하므로 이 파라미터는 무시됩니다.
    ...
    if verbose:
        print("=" * 80)
        print("부피 측정 통합 프로세스 시작")
        print("=" * 80)
        print(f"입력 파일: {depth_file_path}")
        print(f"마스크 축소: {shrink_pixels} 픽셀")
        print("주의: linear_RBF는 평균 깊이 기반 스케일을 사용합니다.")
        print("=" * 80)
...
def compare_results(all_results):
    ...
    if any(actual_volumes):
        ...
        ax4.set_title('RBF 보간: 실제 vs 측정')
        ax4.legend()
        ax4.grid(True, alpha=0.3)
    
    plt.tight_layout()
    output_path = 'volume_comparison_all_cups.png'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='depth 데이터로부터 부피를 측정합니다.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
예제:
  # 단일 파일 처리
  python intergrate_volume.py ./data/413_dojagi.npy
  
  # data 폴더의 모든 파일 배치 처리
  python intergrate_volume.py --batch
  
  # 배치 처리 (옵션 지정)
  python intergrate_volume.py --batch --shrink-pixels 5
        """
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=87,
        help='컵의 실제 직경 (mm), 기본값: 87 (단일 파일 처리 시만 사용)'
    )
