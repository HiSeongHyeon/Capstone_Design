...
try:
    class DummyVolumeEstimator:
        ...
        def predict(self, data_path):
            print(f"[시스템] 데이터 분석 중: {data_path}")
            return {'volume_ml': 350.0} # 테스트용 반환값
    try:
...
class ModernDispenserUI(QMainWindow):
    ...
    def capture_depth_image(self, duration=0.3):
        ...
        if captured_frames:
            ...
            print(f"[카메라] {len(captured_frames)} 프레임 평균 저장 완료: {self.TEMP_DATA_PATH}")
            return True
        else:
            print("[오류] 수집된 프레임이 없습니다.")
            return False
    ...
    def send_to_arduino(self, volume):
        ...
        if self.arduino and self.arduino.is_open:
            try:
                ...
                print(f"[시리얼] 아두이노로 데이터 전송: {msg.strip()}")
            except Exception as e:
                print(f"[시리얼] 전송 실패: {e}")
                # 연결이 끊어졌을 경우 재연결 시도 로직을 넣을 수도 있음
        else:
    ...
    def process_dispense_sequence(self):
        ...
        if self.estimator:
            if os.path.exists(self.TEMP_DATA_PATH):
                ...
                print("[시스템] 스캔 데이터가 없습니다. (더미 값 사용)")
                predicted_capacity = 350.0 
        else:
            print("[시스템] 추론 엔진이 없습니다. (더미 값 사용)")
            predicted_capacity = 350.0

        # 3. 출수량 계산
        if self.current_mode == 'RATIO':
            final_volume_ml = predicted_capacity * self.current_value
            print(f"[출수] 모드: RATIO ({self.current_value*100}%) | 컵: {predicted_capacity}ml -> 출수: {final_volume_ml:.1f}ml")
